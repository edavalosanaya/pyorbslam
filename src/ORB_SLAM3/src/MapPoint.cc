...
MapPoint::MapPoint():
    ...
    mpReplaced(shared_ptr<MapPoint>())
{
    mpReplaced = shared_ptr<MapPoint>();
}
...
MapPoint::MapPoint(const Eigen::Vector3f &Pos, shared_ptr<KeyFrame> pRefKF, Map* pMap):
    ...
    mpReplaced(shared_ptr<MapPoint>()), mfMinDistance(0), mfMaxDistance(0), mpMap(pMap),
...
MapPoint::MapPoint(const Eigen::Vector3f &Pos, Map* pMap, Frame* pFrame, const int &idxF):
    ...
    mnCorrectedReference(0), mnBAGlobalForKF(0), mpRefKF(static_cast<shared_ptr<KeyFrame>>(NULL)), mnVisible(1),
...
void MapPoint::PreSave(set<shared_ptr<KeyFrame>>& spKF,set<shared_ptr<MapPoint>>& spMP)
{
    mBackupReplacedId = -1;
    ...
    for(std::map<shared_ptr<KeyFrame>,std::tuple<int,int> >::const_iterator it = mObservations.begin(), end = mObservations.end(); it != end; ++it)
    {
...
void MapPoint::PostLoad(map<long unsigned int, shared_ptr<KeyFrame>>& mpKFid, map<long unsigned int, shared_ptr<MapPoint>>& mpMPid)
{
    ...
    {
        map<long unsigned int, shared_ptr<MapPoint>>::iterator it = mpMPid.find(mBackupReplacedId);
        if (it != mpMPid.end())
