...
namespace ORB_SLAM3
    ...
    Map::Map():mnMaxKFid(0),mnBigChangeIdx(0), mbImuInitialized(false), mnMapChange(0), mpFirstRegionKF(shared_ptr<KeyFrame>()),
    ...
    Map::Map(int initKFid):mnInitKFid(initKFid), mnMaxKFid(initKFid),/*mnLastLoopKFid(initKFid),*/ mnBigChangeIdx(0), mIsInUse(false),
                        mHasTumbnail(false), mbBad(false), mbImuInitialized(false), mpFirstRegionKF(shared_ptr<KeyFrame>()),
    ...
    {
        int nMPWithoutObs = 0;
        for(shared_ptr<MapPoint> pMPi : mspMapPoints)
        {
            if(!pMPi || pMPi->isBad())
                continue;
            ...
            map<shared_ptr<KeyFrame>, std::tuple<int,int>> mpObs = pMPi->GetObservations();
            for(map<shared_ptr<KeyFrame>, std::tuple<int,int>>::iterator it= mpObs.begin(), end=mpObs.end(); it!=end; ++it)
            {
        ...
        for(shared_ptr<KeyFrame> pKFi : mspKeyFrames)
        {
            if(!pKFi || pKFi->isBad())
                continue;
    ...
    void Map::PostLoad(KeyFrameDatabase* pKFDB, ORBVocabulary* pORBVoc/*, map<long unsigned int, shared_ptr<KeyFrame>>& mpKeyFrameId*/, map<unsigned int, GeometricCamera*> &mpCams)
    {
        ...
        map<long unsigned int,shared_ptr<MapPoint>> mpMapPointId;
        for(shared_ptr<MapPoint> pMPi : mspMapPoints)
        {
            if(!pMPi || pMPi->isBad())
                continue;
        ...
        map<long unsigned int, shared_ptr<KeyFrame>> mpKeyFrameId;
        for(shared_ptr<KeyFrame> pKFi : mspKeyFrames)
        {
            if(!pKFi || pKFi->isBad())
                continue;
