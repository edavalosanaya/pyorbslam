...
namespace ORB_SLAM3
{
                                                 ...
                                                 int &nNumCoincidences, std::vector<shared_ptr<MapPoint> > &vpMPs, std::vector<shared_ptr<MapPoint> > &vpMatchedMPs)
    {
        ...
        for(shared_ptr<KeyFrame>  pKFi : vpBowCand)
        {
            ...
            std::vector<std::vector<shared_ptr<MapPoint> > > vvpMatchedMPs;
            vvpMatchedMPs.resize(vpCovKFi.size());
            std::set<shared_ptr<MapPoint> > spMatchedMPi;
            int numBoWMatches = 0;

            shared_ptr<KeyFrame>  pMostBoWMatchesKF = pKFi;
            int nMostBoWNumMatches = 0;

            std::vector<shared_ptr<MapPoint> > vpMatchedPoints = std::vector<shared_ptr<MapPoint> >(mpCurrentKF->GetMapPointMatches().size(), static_cast<shared_ptr<MapPoint> >(NULL));
            std::vector<shared_ptr<KeyFrame> > vpKeyFrameMatchedMP = std::vector<shared_ptr<KeyFrame> >(mpCurrentKF->GetMapPointMatches().size(), static_cast<shared_ptr<KeyFrame> >(NULL));
            ...
            for(int j=0; j<vpCovKFi.size(); ++j)
            {
                ...
                {
                    shared_ptr<MapPoint>  pMPi_j = vvpMatchedMPs[j][k];
            ...
            if(numBoWMatches >= nBoWMatches) // TODO pick a good threshold
            {
                ...
                if(bConverge)
                {
                    ...
                    set<shared_ptr<KeyFrame> > spCheckKFs(vpCovKFi.begin(), vpCovKFi.end());
                    ...
                    vector<shared_ptr<KeyFrame> > vpKeyFrames;
                    for(shared_ptr<KeyFrame>  pCovKFi : vpCovKFi)
                    {
                        for(shared_ptr<MapPoint>  pCovMPij : pCovKFi->GetMapPointMatches())
                        {
                    ...
                    vector<shared_ptr<MapPoint> > vpMatchedMP;
                    vpMatchedMP.resize(mpCurrentKF->GetMapPointMatches().size(), static_cast<shared_ptr<MapPoint> >(NULL));
                    vector<shared_ptr<KeyFrame> > vpMatchedKF;
                    vpMatchedKF.resize(mpCurrentKF->GetMapPointMatches().size(), static_cast<shared_ptr<KeyFrame> >(NULL));
                    ...
                    if(numProjMatches >= nProjMatches)
                    {
                        ...
                        if(numOptMatches >= nSim3Inliers)
                        {
                            ...
                            if(numProjOptMatches >= nProjOptMatches)
                            {
                                ...
                                for(shared_ptr<MapPoint>  pMPi : vpMatchedMP)
                                {
                                    if(!pMPi || pMPi->isBad())
                                    {
                                        continue;
                                ...
                                vector<shared_ptr<KeyFrame> > vpCurrentCovKFs = mpCurrentKF->GetBestCovisibilityKeyFrames(nNumCovisibles);
